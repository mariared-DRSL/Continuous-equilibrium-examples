...
function [as,ks,history,exitflag,lambda] = FindRest_InternalSprings_Scissor(ThetaA,ThetaB,ThetaC,ThetaD,Grav) 
...
% ObjFun = @(r)mean(sum(abs(diff(transpose((1/2)*r(5)*(ThetaA2-transpose(r(1)-ang)).^2 + (1/2)*r(6)*(ThetaB2-transpose(r(2)-ang)).^2 + (1/2)*r(7)*(ThetaC2-transpose(r(3)-ang)).^2 + (1/2)*r(8)*(ThetaD2-transpose(r(4)-ang)).^2 + transpose(Grav))))));
